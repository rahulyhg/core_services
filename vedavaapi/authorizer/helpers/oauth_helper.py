...
class OAuthClient(object):
    ...
    # step1
    def redirect_for_authorization(self, callback_url, state, scope=None, response_type=None):
        pass
...
class FacebookClient(OAuthClient):
    ...
    def redirect_for_authorization(self, callback_url, state, scope=None, response_type=None):
        params = {
            'response_type': response_type or 'code granted_scopes',
            'client_id': self.client_details['client_id'],
            'redirect_uri': callback_url,
            'scope': scope or ','.join(['email']),
            'state': state
        }
...
class VedavaapiClient(OAuthClient):
    provider_name = 'vedavaapi'

    userinfo_api_endpoint = 'http://localhost:5000/{org_name}/accounts/oauth/v1/resolve_token'

    def __init__(self, config):
        super(VedavaapiClient, self).__init__(config)
        self.org_name = config['org_name']
        client_secret_file_path = config['client_secret_file_path']
        client_secret_json = json.load(open(client_secret_file_path))

        print(self.userinfo_api_endpoint)
        self.userinfo_api_endpoint = self.userinfo_api_endpoint.format(org_name=self.org_name)
        print(self.userinfo_api_endpoint)
    ...
    def redirect_for_authorization(self, callback_url, state, scope=None, response_type=None):
        ...
        vedavaapi_auth_furl = furl(self.client_details['auth_uri'].format(org_name=self.org_name))
        vedavaapi_auth_furl.args.update(params)
        return flask.redirect(vedavaapi_auth_furl.url)
    ...
    def exchange_code_for_access_token(self, auth_code, registered_callback_url):
        ...
        atr = requests.post(self.client_details['token_uri'].format(org_name=self.org_name), data=request_data)
        return atr.json()
...
OauthClientsRegistry.register_client_class(VedavaapiClient)
